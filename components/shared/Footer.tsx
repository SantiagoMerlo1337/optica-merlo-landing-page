...
const Footer = () => {
  return (
    <footer className='text-white background-custom-gradient pb-8 pt-4 mt-14'>
                        ...
                        socialMedia.map((item)=>(
                          <li key={item.label}>
                            <Link href={item.url} target="_blank">
                              <Image width={36} height={36} className="invert-colors" alt={`${item.label}} icon`} src={String(item.imgUrl)}/>
                            </Link>
                          </li>
                        ))
                      }
        ...
        <div className="flex-center mt-6">
            <Image width={32} height={32} alt="Optica Merlo logo" src="/assets/images/logo-png.png" className="invert-colors" />
        </div>
    </footer>
  )
}
